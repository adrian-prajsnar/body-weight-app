...
- BMI uses height effective on each weight entry date (`src/height.ts` → `getHeightAtDate`); weight and height are stored in kg/cm in Supabase; display uses metric or imperial via Profile → Units (System follows device `measurementSystem`, US → lb/ft/in)
